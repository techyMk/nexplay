...
- **Mobile Lighthouse perf is 69 / 100.** Desktop is 98. **A11y, BP, SEO are all 100** on both profiles. The remaining mobile perf gap is React-hydration TBT (~500 ms) — the next lever is lazy-importing Supabase on routes that don't need it. See `docs/lighthouse/` for the trend.
