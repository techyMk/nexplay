...
import { PongRoomClient } from "./Room";

export const metadata = { title: "Pong Room — Nexplay" };

export default async function PongRoomPage({
  params,
}: {
  ...
  if (!user) {
    redirect(`/login?next=/multiplayer/pong/${upper}`);
  }
  ...
  if (!room || room.game_slug !== "pong") notFound();
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 sm:px-6 py-6 md:py-10">
      <div className="mb-4">
        <BackButton fallback="/multiplayer/pong" label="Lobby" />
      </div>
