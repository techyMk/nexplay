...
export const metadata = { title: "Multiplayer Checkers — Nexplay" };
...
async function createRoomAction() {
  ...
  if (!user) redirect("/login?next=/multiplayer/checkers");
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (!error) redirect(`/multiplayer/checkers/${code}`);
...
export default async function CheckersLobbyPage() {
    ...
    <div className="mx-auto max-w-2xl px-4 sm:px-6 py-8 md:py-12">
      ...
      <div className="text-center mb-10">
        ...
        <p className="text-[var(--muted)]">
          Real-time 1-vs-1. Mandatory captures, multi-jumps, kings.
        </p>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-4 mb-8">
        <form action={createRoomAction}>
          <button
            type="submit"
            className="w-full h-32 rounded-2xl bg-gradient-to-br from-stone-700 to-red-700 text-white p-6 text-left hover:scale-[1.02] transition-transform"
          >
        ...
        <div className="rounded-2xl bg-[var(--surface)] border border-[var(--border)] p-6">
          ...
          <JoinForm basePath="/multiplayer/checkers" />
        </div>
      </div>
