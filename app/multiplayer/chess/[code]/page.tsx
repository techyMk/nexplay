...
import { ChessRoomClient } from "./Room";

export const metadata = { title: "Chess Room — Nexplay" };
...
export default async function ChessRoomPage({
  ...
  if (!user) {
    redirect(`/login?next=/multiplayer/chess/${upper}`);
  }
  ...
  if (!room || room.game_slug !== "chess") notFound();

  // Auto-join as guest if the room is waiting and we're not the host.
  if (
    !room.guest_user_id &&
  ...
  const guest = room.guest_user_id
    ? profileById.get(room.guest_user_id)
    : null;
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 sm:px-6 py-6 md:py-10">
      <div className="mb-4">
        <BackButton fallback="/multiplayer/chess" label="Lobby" />
      </div>
