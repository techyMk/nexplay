...
import { C4RoomClient } from "./Room";

export const metadata = { title: "Connect Four Room — Nexplay" };

export default async function C4RoomPage({
  params,
}: {
  ...
  if (!user) {
    redirect(`/login?next=/multiplayer/connect-four/${upper}`);
  }
  ...
  if (!room || room.game_slug !== "connect-four") notFound();

  // Auto-join the room as guest if there's a slot
  if (
    !room.guest_user_id &&
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 sm:px-6 py-6 md:py-10">
      <div className="mb-4">
        <BackButton fallback="/multiplayer/connect-four" label="Lobby" />
      </div>
